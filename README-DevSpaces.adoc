...
Open two terminal windows, referred to as *T2* and *T3* and change to the folder where this repository has been cloned.
...
Open quay from *T2* and check the image is there:
...
let's create a new `BuildConfig` this time is a binary build, from *T3*:
...
Then start the binary build using `target/quarkus-app` as a binary asset, from *T3*:
...
Copy the new image to the enterprise registry as before. Run this command from *T2*:
