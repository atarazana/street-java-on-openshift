...
Now, run this command in *T1* and *T2* before running the demo:
...
NOTE: Optionally run the React UI, `DANGEROUSLY_DISABLE_HOST_CHECK=true` makes it possible to access the dev server running in DevSpaces
...
In *T2* run the app in `prod` profile:

[source,sh,attributes]
----
FRUIT_SERVICE_URL=http://fruit-service.${PROJECT_NAME}:8080 \
  mvn quarkus:dev -Dquarkus.profile=prod -Dquarkus.log.console.json=false
----

NOTE: Open `jaeger` UI from *T1* to show that this service is reachable from `localhost`:

[source,sh,attributes]
----
echo "Open this: https://$(oc get route jaeger-all-in-one-inmemory -n street-java-infra -o jsonpath='{.spec.host}')"
----

[#round-3]
== Round 3 - Flying Kick to OCP

Type 'q' in *T2* and deploy our app directly from quarkus, using the openshift extension:
...
Before delete all the resources created... from *T2*.
...
And upgrade the helm chart setting image repository and tag to the ones we had been using before from *T1*.
...
Prepare environment for registry, image and tag, from *T1*:
...
Open quay from *T1* and check the image is there:

[source,sh,attributes]
----
echo "Open: https://$(oc get route myregistry-quay -n quay-system -o jsonpath='{.spec.host}')/${USERNAME}/${PROJECT_ID}-${ARTIFACT_ID}:${GIT_HASH}"
----
...
[source,sh,attributes]
----
oc start-build bc/fruit-gateway
----

Log in the enterprise registry:

[source,sh,attributes]
----
skopeo login -u $(oc whoami) -p openshift ${REGISTRY}
----

Log in the internal registry:

[source,sh,attributes]
----
skopeo login -u $(oc whoami) -p $(oc whoami -t) --tls-verify=false image-registry.openshift-image-registry:5000
----

Copy image to enterprise registry:

[source,sh,attributes]
----
skopeo copy --src-tls-verify=false \
  docker://image-registry.openshift-image-registry:5000/${PROJECT_NAME}/fruit-gateway:latest \
  docker://${REGISTRY}/${USERNAME}/${PROJECT_ID}-${ARTIFACT_ID}:${GIT_HASH}
----

Open quay from *T1* and check the image is there. Log in with these credentials:
...
=== Resume after pushing the new image
...
Open quay from *T1* and create a robot account called `cicd`, give it `Read` permission on the `street-java-fruit-gateway` repository:

[source,sh,attributes]
----
echo "Open Quay: https://$(oc get route myregistry-quay -n quay-system -o jsonpath='{.spec.host}')"
----
...
Let's check the status of the whole system through the `config` api exposed by our gateway. Although maybe you already notice errors in jaeger before... Use *T1* to run this command:
...
Let's add an extension to help us generating Prometheus metrics. Run this in *T2*.
...
As we did in before, let's test locally our changes from *T2*.
...
You can stop quarkus dev with `q`. Now from *T1* let's build the new image locally:
...
let's create a new `BuildConfig` this time is a binary build, from *T2*:

[source,sh,attributes]
----
oc new-build --image-stream=openshift/ubi8-openjdk-17:1.12 --name fruit-gateway-bin --env-file=.s2i/environment --to=fruit-gateway --binary=true
----

Then start the binary build using `target/quarkus-app` as a binary asset, from *T2*:

[source,sh,attributes]
----
mvn package -DskipTests
oc start-build fruit-gateway-bin --from-dir=target/quarkus-app --follow=true --wait
----

Copy the new image to the enterprise registry as before. Run this command from *T1*:
...
[source,sh,attributes]
----
echo "Open Metrics: https://console-openshift-console.apps.${CLUSTER_DOMAIN}/dev-monitoring/ns/${PROJECT_NAME}/metrics"
----
...
[source,sh,attributes]
----
echo "Open Alerts: https://console-openshift-console.apps.${CLUSTER_DOMAIN}/dev-monitoring/ns/${PROJECT_NAME}/alerts"
----
