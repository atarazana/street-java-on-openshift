...
let's create a new `BuildConfig` this time is a binary build, and start start it, from *T3*:
...
oc start-build fruit-gateway-bin --from-dir=target --follow=true --wait
----
